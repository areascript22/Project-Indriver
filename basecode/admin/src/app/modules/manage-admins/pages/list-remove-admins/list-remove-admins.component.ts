...
@Component({
  selector: 'app-list-remove-admins',
  templateUrl: './list-remove-admins.component.html',
  styleUrl: './list-remove-admins.component.css',
})
export class ListRemoveAdminsComponent {
  items: any[] = [];
  accessOptions = [
  ...
  ngOnInit(): void {
    this.firestoreService.getDrivers('g_user').subscribe((data) => {
      this.items = data.filter((val) => val.role.includes('admin'));
